-- Use this version if your workspace does not have a `main` catalog.
-- If you prefer a different catalog, replace `hive_metastore.demo_dash_genie`
-- with `<your_catalog>.demo_dash_genie` throughout this file and set DEMO_SCHEMA
-- to the same value in .env and app.yaml.

CREATE SCHEMA IF NOT EXISTS hive_metastore.demo_dash_genie;

CREATE OR REPLACE TABLE hive_metastore.demo_dash_genie.customers AS
SELECT
...
CREATE OR REPLACE TABLE hive_metastore.demo_dash_genie.orders AS
SELECT
...
CREATE OR REPLACE TABLE hive_metastore.demo_dash_genie.support_tickets AS
SELECT
...
CREATE OR REPLACE VIEW hive_metastore.demo_dash_genie.orders_enriched AS
SELECT
  o.order_id,
  o.customer_id,
  c.customer_name,
...
FROM hive_metastore.demo_dash_genie.orders o
JOIN hive_metastore.demo_dash_genie.customers c
  ON o.customer_id = c.customer_id;

CREATE OR REPLACE VIEW hive_metastore.demo_dash_genie.support_tickets_enriched AS
SELECT
  t.ticket_id,
  t.customer_id,
  c.customer_name,
  ...
  t.csat_score
FROM hive_metastore.demo_dash_genie.support_tickets t
JOIN hive_metastore.demo_dash_genie.customers c
  ON t.customer_id = c.customer_id;

CREATE OR REPLACE VIEW hive_metastore.demo_dash_genie.customer_health_360 AS
WITH revenue AS (
  SELECT
    customer_id,
  ...
  FROM hive_metastore.demo_dash_genie.orders_enriched
...
support AS (
  ...
  FROM hive_metastore.demo_dash_genie.support_tickets_enriched
  GROUP BY customer_id
)
SELECT
  c.customer_id,
  ...
  CASE
    ...
    ELSE 'Low'
  END AS risk_bucket
FROM hive_metastore.demo_dash_genie.customers c
LEFT JOIN revenue r ON c.customer_id = r.customer_id
LEFT JOIN support s ON c.customer_id = s.customer_id;

SELECT *
FROM hive_metastore.demo_dash_genie.customer_health_360
