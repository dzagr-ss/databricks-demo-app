-- PDF default setup. Use this only if SHOW CATALOGS includes `main`.
-- If your workspace does not have `main`, run setup_demo_data_hive_metastore.sql
-- or replace `main.demo_dash_genie` with an existing `<catalog>.demo_dash_genie`.

CREATE SCHEMA IF NOT EXISTS main.demo_dash_genie;

CREATE OR REPLACE TABLE main.demo_dash_genie.customers AS
SELECT
...
CREATE OR REPLACE TABLE main.demo_dash_genie.orders AS
SELECT
...
CREATE OR REPLACE TABLE main.demo_dash_genie.support_tickets AS
SELECT
...
CREATE OR REPLACE VIEW main.demo_dash_genie.orders_enriched AS
SELECT
  o.order_id,
  o.customer_id,
  c.customer_name,
...
FROM main.demo_dash_genie.orders o
JOIN main.demo_dash_genie.customers c
  ON o.customer_id = c.customer_id;

CREATE OR REPLACE VIEW main.demo_dash_genie.support_tickets_enriched AS
SELECT
  t.ticket_id,
  t.customer_id,
  c.customer_name,
  ...
  t.csat_score
FROM main.demo_dash_genie.support_tickets t
JOIN main.demo_dash_genie.customers c
  ON t.customer_id = c.customer_id;

CREATE OR REPLACE VIEW main.demo_dash_genie.customer_health_360 AS
WITH revenue AS (
  SELECT
    customer_id,
  ...
  FROM main.demo_dash_genie.orders_enriched
...
support AS (
  ...
  FROM main.demo_dash_genie.support_tickets_enriched
  GROUP BY customer_id
)
SELECT
  c.customer_id,
